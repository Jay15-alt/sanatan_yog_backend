...
import { PeethService, ShakhaService, SubShakhaService, TeamService } from '../services/organizationService';
...
router.get('/shakha', async (req: Request, res: Response) => {
  try {
    ...
    const list = peethId ? await ShakhaService.findByPeeth(peethId) : [];
    sendSuccess(res, list);
  } catch {
    sendError(res, 'Internal error.', 500);
  }
